...
- In our approach,if the properties of miRNA:target interactions are degreed according to experimental studies, these factors takes into account in the regulations of miRNA:target interactions. But these factors are optionally evaluated. So, if the interaction factors are not specified, they are not taken into account and the system are ran based on miRNA and target amounts. (#TODO we can remove this item, does not explain factors effecting perturbation. also kind of reduntant with previous item)
...
* **perturbation efficiency** : mean of percentage of expression changes of all elements in the network 
...
In the example below, "Gene17" is up-regulated 3-fold in *midsamp* dataset where `Energy` and `seeds` columns are used for calculating affinity effect and `targeting_region` columns is used for calculating degradation effect. The network will be iterated over 30 times and 0.1 percent of change will be taken as threshold (#TODO below you said: "The `limit` symbolizes the percentage of minimum change of node expressions." you mean for single node, right?) and number of disturbed nodes will be counted.
...
If you are insterested in testing various fold change values of a given node, then we can use `map` (actually parallelized version `future_map`) to run function for set of input values.
...
If you're insterested in screening nodes instead of fold changes then you don't have to write a complicated `map` command, there's already a function available for that purpose.
