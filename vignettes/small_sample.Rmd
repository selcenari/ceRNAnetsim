...
- miRNA efficiency in steady-state is calculated by taking into account of expression distribution and effecting factors. The **optional** factors might have two types of effect; 1) affinity, 2) degradation effect. Any column which has effect on affinity should be provided as a vector to `aff_factor` argument. 
...
With these functions (**update_nodes**, **update_how**) the current value(s) of element on the network, is updated in edge variables. After that, if **update_nodes** function is applied (as shown in following chunk), the change(s) can be seen in node variables.


## Update the node variables with edge variables.

The node variables updates after the change in edge. This step are applied for easy tracking.
