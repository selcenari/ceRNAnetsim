...
- The calculated values in edge are carried to node data through `update_nodes()` function.
...
Load the `new_count` dataset (provided with package sample data) in which expression level of Gene2 is increased 2 fold (from 10,000 to 20,000).
...
The functions `update_variables()` and `update_how()` updates edge data. After that, `update_nodes()` function should be applied in order to reflect changes in edge data over to node data. In other words, if there's a change in edge data, nodes can be updated accordingly with `update_nodes()` function.
...
(#TODO update_nodes function is kind of obsolete, and burden for user, I think update_nodes function should be silently called within update_how and update_variables functions, it should not be called by user)
...
(#TODO gene5 stays at 5000 and gene6 stays at 10000 for 10 cycles, is that normal?)

(#TODO in convenient_iteration.Rmd vignette, the animated gif /gifs/con_iter_chunk3.gif shows there's change in expression at each cycle, but actually there's no change after cycle 3 for Gene4, is it due to `limit=0` case?s)
...
> The animated gif is generated by combining gif of each step, so user is responsible for generating the animated gif at terminal or online service, the function only generates images at each cycle as frames of an animation (#TODO am I saying this right?)
