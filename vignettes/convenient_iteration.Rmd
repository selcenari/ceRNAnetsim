...
In the simple interaction network like *minsamp*, the ripple effect could be observed when expression level of *Gene4* changes and subsequently effecting other genes. (#TODO didnt understand this sentence: The non-complex interaction can be facilitated to provide the steady-state, after the network is disturbed.)
...
For example, in *minsamp* dataset, the steady-state is occured at iteration-14, as seen above. However, when network is larger and interactions are more complex, the number of iterations required to reach steady-state may increase. While at cycle 14 *minsamp* dataset has reached steady-state, the *midsamp* (middle sized sample) dataset has not reached steady-state after 15 cycles. In the example below, in *midsamp* data, *Gene17* is upregulated 2 fold as a trigger and simulation is run for 15 cycles.

(#TODO how should readed know the steady-state is not reached, are they supposed to look for green circles? indicating genes without change?)
...
(#TODO wouldn't this function be more modular if function works in this manner:
midsamp %>%
  ...
  update_nodes() %>% 
  **find_iteration(.iter=50, limit=0)**
