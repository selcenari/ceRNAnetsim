...
find_iteration <- function(df, competing_count, miRNA_count, aff_factor=dummy, deg_factor=dummy, node_name, how, .iter = 100, limit= 0.2){
  ...
  df%>%
    ...
    simulate(cycle = .iter)->result_100
     ...
     result_100%E>%
      ...
      distinct()%>%
      mutate(non_zero = ifelse(dif>limit, 1, 0))%>%
